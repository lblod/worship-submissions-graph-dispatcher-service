...
console.log(
  `The healing pool will consist of ${NUMBER_OF_HEALING_QUEUES} queues`,
);
...
app.get("/manual-dispatch", async function (req, res) {
  if (req.query.subject) {
    ...
    console.log(
      `Dispatching all submissions (again) from GRAPH ${DISPATCH_SOURCE_GRAPH}`,
    );
    const submissions = await getSubmissions({
      inGraph: DISPATCH_SOURCE_GRAPH,
    });
...
app.get("/heal-submission", async function (req, res) {
  ...
  if (req.query.sentDateSince) {
    let sentDateSince = new Date(req.query.sentDateSince)
      .toISOString()
      .split("T")[0];
    console.log(
      `Received "?sentDateSince" paramater: ${req.query.sentDateSince}`,
    );
    console.log(`Converted to short date: ${sentDateSince}`);
    console.log(
      `Healing will be applied to submissions with sent date >= ${sentDateSince}`,
    );
  ...
  try {
    ...
    for (const type of types) {
      try {
        ...
        console.error(
          `Error while processing a subject ${subject}: ${e.message ? e.message : e}`,
        );
    ...
    console.error(
      `Error while processing a subject: ${e.message ? e.message : e}`,
    );
    await sendErrorAlert({
...
async function dispatch(submission, visitedSubmissions = new Set()) {
  ...
  const applicableRules = dispatchRules.filter((r) => {
    const isOneSubmissionTypeIncluded = submissionInfo.submissionTypes.includes(
      r.documentType,
    );
...
async function dispatchSubmissionToGraphs(submission, targetGraphs) {
  ...
  for (const subject of subjectsToDispatch) {
    for (const graph of targetGraphs) {
      subjectsAndTargetGraphsCartProduct.push({ subject, graph });
    }
  }

  // Count number of triples per subject
  let counts = await getGraphsAndCountForSubjects(subjectsToDispatch, [
    DISPATCH_SOURCE_GRAPH,
    DISPATCH_FILES_GRAPH,
  ]);
  ...
  subjectsAndTargetGraphsCartProduct.forEach((e) => {
    e.count = counts.find((f) => f.subject === e.subject)?.count ?? 0;
  });
  ...
  for (const allSub of subjectsAndTargetGraphsCartProduct) {
    const found = subjectsAndGraphs.find(
      (e) => e.subject === allSub.subject && e.graph === allSub.graph,
    );
    if (found) {
...
async function getAllRelatedSubjects(submission) {
  ...
  for (const config of exportConfig) {
    ...
    addMany(relatedSubjects, ...subjects);
  }
